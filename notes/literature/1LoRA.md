...
- **核心公式**：ΔW=b1T，其中 1\mathbb{1} 1 是全為 1 的固定向量（不訓練），b∈Rdb \in \mathbb{R}^d b∈Rd 是唯一可訓練參數
- **跟 LoRA 的差異**：LoRA 用兩個可訓練矩陣 A、BA、B A、B 做「壓縮→解壓縮」；1LoRA 把壓縮那一步固定死（用加總取代學習），只留解壓縮向量 bb b 可訓練
- **參數量**：dd d（vs. rank-1 LoRA 的 k+dk+d k+d）
- **推論零負擔**：訓練完 ΔW\Delta W ΔW 可直接合併回原始權重 W0W_0 W0​，跟 LoRA 一樣不影響推論速度
